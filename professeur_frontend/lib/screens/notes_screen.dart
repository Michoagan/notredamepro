...
class _NotesScreenState extends State<NotesScreen> {
  ...
  bool _canEditToday = true;
  List<EmploiDuTemps> _monEmploiDuTemps = [];

  int _totalEvalues = 0;
  int _nombreReussite = 0;
  int _nombreEchec = 0;
  double _pourcentageReussite = 0.0;
  double _pourcentageEchec = 0.0;
  double _moyenneGeneraleClasse = 0.0;
  ...
  Future<void> _loadClasses() async {
    ...
    try {
      ...
      try {
        ...
        // Ignorer l'erreur emploi du temps ici
      }
  ...
  Future<void> _loadEleves() async {
    ...
    try {
      ...
      // ── Vérification EmploiDuTemps ───────────────────────────────────
      final dayNames = ['Lundi', 'Mardi', 'Mercredi', 'Jeudi', 'Vendredi', 'Samedi', 'Dimanche'];
      final todayIso = DateTime.now().weekday;
      final dayName = dayNames[todayIso - 1];

      final hasClassToday = _monEmploiDuTemps.any((slot) => 
          slot.classeId == _selectedClasse!.id && slot.jour == dayName);
      
      _canEditToday = hasClassToday;
      // ─────────────────────────────────────────────────────────────────
  ...
  Future<void> _loadMoyennes() async {
    ...
    try {
      ...
      setState(() {
        ...
        _totalEvalues = _moyennes.length;
        if (_totalEvalues > 0) {
          _nombreReussite = _moyennes.where((m) => (m.moyenneTrimestrielle ?? 0) >= 10).length;
          _nombreEchec = _totalEvalues - _nombreReussite;
          _pourcentageReussite = (_nombreReussite / _totalEvalues) * 100;
          _pourcentageEchec = (_nombreEchec / _totalEvalues) * 100;
          
          double sum = _moyennes.fold(0, (prev, m) => prev + (m.moyenneTrimestrielle ?? 0));
          _moyenneGeneraleClasse = sum / _totalEvalues;
        } else {
          _nombreReussite = 0;
          _nombreEchec = 0;
          _pourcentageReussite = 0.0;
          _pourcentageEchec = 0.0;
          _moyenneGeneraleClasse = 0.0;
        }
    ...
    return Scaffold(
      appBar: PremiumAppBar(
        ...
        subtitle: _selectedClasse != null
            ? '${_selectedClasse!.displayName} • ${_getNoteTypeDisplay()}'
            : null,
        actions: [
          if (_isLoading)
            const Padding(
          ...
          : SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          DropdownButtonFormField<int>(
                            ...
                            onChanged: (trimestre) {
                              setState(() => _selectedTrimestre = trimestre!);
                              _loadEleves();
                            },
                            decoration: InputDecoration(
                              labelText: 'Trimestre',
                  ...
                  if (_selectedClasse != null && !_canEditToday)
                    Container(
                      ...
                      child: Row(children: [
                        ...
                        const Expanded(
                          child: Text(
                            'Saisie verrouillée : Vous n\'avez pas de cours prévu avec cette classe aujourd\'hui.',
                            style: TextStyle(fontWeight: FontWeight.w600, color: AppTheme.textPrimary),
                          ),
                        ),
                      ]),
                    )
                  else if (_selectedClasse != null && _eleves.isNotEmpty)
                    Card(
                      child: Padding(
                        padding: const EdgeInsets.all(24),
                        child: Column(
                            ...
                            SizedBox(
                              ...
                              child: OutlinedButton.icon(
                                ...
                                label: const FittedBox(
                                  ...
                                  child: Text('Calculer et afficher les moyennes'),
                                ),
                              ),
                            ),
                          ],
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Container(
                                ...
                                child: Column(
                                  children: [
                                    const Text('Statistiques de la classe', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: AppTheme.info)),
                                    ...
                                    Wrap(
                                      ...
                                      runSpacing: 16,
                                      children: [
                                        _buildStatItem('Effectif', '$_totalEvalues', Colors.blue),
                                        _buildStatItem('Moy. Classe', _moyenneGeneraleClasse.toStringAsFixed(2), Colors.purple),
                                        _buildStatItem('Réussite', '$_nombreReussite (${_pourcentageReussite.toStringAsFixed(1)}%)', Colors.green),
                                        _buildStatItem('Échec', '$_nombreEchec (${_pourcentageEchec.toStringAsFixed(1)}%)', Colors.red),
                                      ],
                                    ),
                                  ],
                                ),
